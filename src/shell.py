"""Интерактивная оболочка с настройками и стартовым сценарием."""

import shlex
import socket

from src.config import ConfigError, load_settings
from src.commands import CommandError, ShellSession, os_user
from src.vfs import VFS, VfsError


def make_prompt(cwd="/"):
    """Сформировать приглашение из имени пользователя и имени узла ОС."""
    location = "~" if cwd == "/" else cwd
    return f"{os_user()}@{socket.gethostname()}:{location}$ "


def execute_line(line, strict=False, session=None):
    """Разобрать и исполнить одну команду в заданном сеансе."""
    if session is None:
        session = ShellSession()
    try:
        words = shlex.split(line, comments=True)
    except ValueError as error:
        report_error(f"ошибка разбора: {error}", strict)
        return True

    if not words:
        return True

    command, *arguments = words
    try:
        return session.execute(command, arguments)
    except CommandError as error:
        report_error(str(error), strict)
        return True


def report_error(message, strict):
    """Напечатать ошибку или передать ее стартовому сценарию."""
    if strict:
        raise CommandError(message)
    print(f"shell: {message}")


def run_startup(path, session=None):
    """Исполнить сценарий, показывая ввод и вывод как в диалоге."""
    if session is None:
        session = ShellSession()
    try:
        lines = path.read_text(encoding="utf-8").splitlines()
    except OSError as error:
        raise CommandError(f"не удалось прочитать сценарий {path}: {error}") from error
    for number, line in enumerate(lines, start=1):
        if not line.strip() or line.lstrip().startswith("#"):
            continue
        print(f"{make_prompt(session.cwd)}{line}")
        try:
            if not execute_line(line, strict=True, session=session):
                return False
        except CommandError as error:
            raise CommandError(f"{path}:{number}: {error}") from error
    return True


def run_repl(session):
    """Читать команды до exit или конца входного потока."""
    while True:
        try:
            line = input(make_prompt(session.cwd))
        except EOFError:
            print()
            return
        except KeyboardInterrupt:
            print()
            continue
        if not execute_line(line, session=session):
            return


def main(argv=None):
    """Прочитать настройки, выполнить сценарий и открыть REPL."""
    try:
        settings = load_settings(argv)
    except ConfigError as error:
        print(f"config: {error}")
        return 1
    print(f"config: {settings.config_path}")
    print(f"vfs: {settings.vfs_path}")
    print(f"startup: {settings.startup_script}")
    vfs = VFS()
    if settings.vfs_path:
        try:
            vfs = VFS.from_zip(settings.vfs_path)
        except VfsError as error:
            print(f"vfs: {error}")
            return 1
        message = vfs.motd()
        if message is not None:
            print(message, end="" if message.endswith("\n") else "\n")
    session = ShellSession(vfs)
    if settings.startup_script:
        try:
            if not run_startup(settings.startup_script, session):
                return 0
        except CommandError as error:
            print(f"startup: {error}")
            return 1
    run_repl(session)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
